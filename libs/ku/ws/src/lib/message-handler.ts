...
export class MessageHandler
  implements Record<KuWs[Channel]['SUB']['PAYLOAD']['subject'], any>
{
  private undefined: NoSubjectHandler;

  private handlers = new Map<
    Channel,
    ((jData: KuWs[Channel]['SUB']['PAYLOAD']) => void)[]
  >([
    ['LEVEL_2', []],
    ['ACCOUNT_BALANCE', []],
  ] satisfies [[Channel, ((jData: KuWs['LEVEL_2']['SUB']['PAYLOAD']) => void)[]], [Channel, ((jData: KuWs['ACCOUNT_BALANCE']['SUB']['PAYLOAD']) => void)[]]]);
   ...
   */
  constructor(noSubjectHandler: NoSubjectHandler) {
    this.undefined = noSubjectHandler;
  }

  public addHandler<T extends Channel>(
    channel: T,
    cb: (jData: KuWs[T]['SUB']['PAYLOAD']) => void
  ) {
  ...
  public rmHandler(channel: Channel, cleaner: number) {
    ...
    const index = channelHandlers.findIndex(
      (cb) => (cb as any).cleaner === cleaner
    );
  ...
  'trade.l2update'(jData: KuWs['LEVEL_2']['SUB']['PAYLOAD']) {
    this.handlers
      .get('LEVEL_2' satisfies Channel)!
      .forEach((cb) => void cb(jData));
  }
   ...
   * But dynamically, from message.subject - OK.
   */
  'account.balance'(jData: KuWs['ACCOUNT_BALANCE']['SUB']['PAYLOAD']) {
    this.handlers
      .get('ACCOUNT_BALANCE' satisfies Channel)!
      .forEach((cb) => void cb(jData));
  }
}
